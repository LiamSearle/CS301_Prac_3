...
public class Parser {
	...
	static void Expression() {
		if (la.kind == bang_Sym || la.kind == not_Sym) {
			Not();
		} else if (la.kind == ident_Sym || la.kind == value_Sym) {
			Term();
			if (StartOf(1)) {
				Operator();
			}
...
public class Errors {
	...
	public static void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 15: s = "invalid Not"; break;
			case 16: s = "invalid Term"; break;
			case 17: s = "invalid Operator"; break;
